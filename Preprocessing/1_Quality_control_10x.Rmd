...
sample_metadata <- read.table("~/Dropbox/Postdoc/2021-05-19_Gastric_IM/Additional_files/Samples.txt", sep = "\t", header = TRUE, row.names = 1)
```



```{r data, eval=TRUE}
folders <- list.files("~/Dropbox/Postdoc/2019-10-29_Gastric_IM/Data_v3/", full.names = TRUE)
folder.names <- list.files("~/Dropbox/Postdoc/2019-10-29_Gastric_IM/Data_v3/", full.names = FALSE)
sample_metadata$Tracking <- paste(sample_metadata$Patient, sample_metadata$Barcode, sample_metadata$Tissue, "out", sep = "_")
...
for(i in 1:length(folders)){
  title.name <- gsub("_out", "",folder.names[i])
  ...
  else
  {
    ...
    colData(cur_sce)$Patient <- sample_metadata[sample_metadata$Tracking == folder.names[i],"Patient"]
    colData(cur_sce)$Tissue <- sample_metadata[sample_metadata$Tracking == folder.names[i],"Tissue"]
    colData(cur_sce)$Batch <- sample_metadata[sample_metadata$Tracking == folder.names[i],"Batch"]
    colData(cur_sce)$Sample_Barcode <- sample_metadata[sample_metadata$Tracking == folder.names[i],"Barcode"]
    colData(cur_sce)$Sample <- gsub("_out", "",folder.names[i])
    ...
    print(ggplot(data.frame(Total = cur_sce$total,
                            MT.prop = MT.prop,
                            MUC5AC = log10(counts(cur_sce)[rowData(cur_sce)$Symbol == "MUC5AC",]))) + 
            geom_point(aes(Total, MT.prop, colour = MUC5AC)) +
            scale_x_log10() +
            scale_color_viridis_c(option = "inferno") +
            annotation_logticks(sides="b") +
            theme_minimal() + 
...
# Save list
saveRDS(sce.list, "~/Dropbox/Postdoc/2021-05-19_Gastric_IM/All_sce_unfiltered_10x.rds")

saveRDS(sce.list.filtered, "~/Dropbox/Postdoc/2021-05-19_Gastric_IM/Final/All_sce_filtered_10x.rds")
```
